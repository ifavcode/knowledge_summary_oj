...
import { editQuestionTestcaseApi, publishQuestionApi, saveQuestionTagApi } from '~/api/QuestionApi';
...
async function publishQuestion() {
  ...
  const res = await publishQuestionApi(questionOptions)
  if (res.code === 200) {
    testcaseList.value.map(v => {
      v.questionCode = res.data?.questionCode
    })
    const res2 = await editQuestionTestcaseApi(testcaseList.value, res.data?.questionCode as string)
    const res3 = await saveQuestionTagApi(res.data?.id as number, tagState.tags)
    if (res2.code === 200 && res3.code === 200) {
      message.success('创建成功')
      router.push('/questions/index')
    } else {
      if (res2.code !== 200) {
        message.warning('测试样例添加失败！')
      }else if (res3.code !== 200) {
        message.warning('标签添加失败！')
      }
    }
  }else{
    message.warning(res.msg || '系统异常')
  }
}
...
<template>
  <div class="my-20 w-[1200px] mx-auto max-sm:w-full">
    <div class="w-[1000px] mx-auto">
      <div class="mb-4">
        ...
        <a-input class="px-[20px]" v-model:value="questionOptions.questionCode" :bordered="false"
          placeholder="请输入题目编号" />
  ...
  <div class="h-[50px] fixed bottom-0 left-0 right-0 w-full border-t border-gray-300 bg-white">
    <div class="w-[1000px] mx-auto flex items-center h-full justify-end">
      <a-button class="w-24" type="primary" @click="publishQuestion">创建</a-button>
    </div>
  </div>
</template>
